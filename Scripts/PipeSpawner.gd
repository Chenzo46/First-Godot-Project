...
func _spawn_pipe():
	...
	with a random y value between -300 and 3oo
'''
func _spawn_pipe():
	var pipes_instance:Node2D = pipes_prefab.instantiate()
	add_child(pipes_instance)
	pipes_instance.position = Vector2(position.x, rand_gen.randf_range(-300,300))
